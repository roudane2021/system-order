...
public class H2OrderPersistenceAdapter implements IOrderPersistenceOutPort {
    ...
    public OrderModel getOrder(Long orderID) {
        // Delegates to findOrderById and throws exception if not found, or handles as per specific contract
        // This implementation assumes getOrder should throw if not found, matching typical repository getById behavior
    ...
    public OrderModel updateOrder(OrderModel orderModel) {
        // Ensure the order exists before attempting an update
        if (orderModel.getId() == null || !orderJpaRepository.existsById(orderModel.getId())) {
            // Or throw a custom domain exception like OrderNotFoundException
            throw new RuntimeException("Order with ID " + orderModel.getId() + " not found, cannot update.");
        }
        OrderEntity orderEntity = persistenceOrderMapper.toEntity(orderModel);
